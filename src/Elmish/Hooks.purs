...
withHooks hooks = runCont (runWriterT hooks) toElem
  where
    ...
    -- `Hook ReactElement`, i.e. every time `mkHook name …` is called within the
    -- block passed to `withHooks`. If there are multiple hooks with the same
    -- name, it could be the case that they can appear in the same place in the
    -- DOM tree (e.g. if there’s a conditional). If that happens, React may
    -- assume that they’re the same component even if they have different
    -- semantics, leading to possibly confusing bugs. For this reason, we log an
    -- error to the console if there are duplicates.
    error names = do
